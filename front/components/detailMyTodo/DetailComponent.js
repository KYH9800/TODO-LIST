...
const DetailComponent = ({ detailTodoList, detailmodal }) => {
  ...
  const doneTodo = () => {
    if (confirm('작업을 완료하시겠습니까?')) {
      console.log('완료'); // dispatch (done: fasle -> true)
    }
  };
  ...
  const notDoneTodo = () => {
    if (confirm('작업을 완료하지 않은 상태로 되돌리겠습니까?')) {
      console.log('미완료'); // dispatch (done: true -> false)
    }
  };
    ...
    <>
      ...
      <ContentBox>
        <div className="created-at">
          <span>업로드일: </span> <span>{detailTodoList.createdAt}</span>
        </div>
        ...
        <div>
          {updateState ? (
            <div>
              <Input type="text" placeholder="주요 내용" />
            </div>
          ) : (
        ...
        {updateState ? (
          <div>
            <Textarea type="text" placeholder="상세 내용" />
          </div>
        ) : (
